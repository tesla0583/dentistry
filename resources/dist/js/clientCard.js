                        ...
                        if (response.code === 1) {
                            ...
                            if (data !== null && data !== '') {
                                ...
                                data.forEach(function (item, key) {
                                    // alert(item.fio);
                                    table.append(
                                        "<tr><td>" + item.fio + "</td><td>" + item.date_pers + "</td><td>" + item.inn + "</td>" +
                                        "<td>" + item.passport + "</td><td>" + item.contact_numb + "</td><td>" + item.client + "</td></tr>"
                                    );
                                });
                            } else {
                                Swal.fire({
        ...
        didOpen: () => {
            ...
            var checkDispenseInterval = setInterval(function () {
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "api.php?requestType=R_PRIV_FIN&absId=" + clId + "&busSessionId=" + get_client_session_id,
                    success: function (response) {
                        if (response.code === 1) {
                            clearInterval(checkDispenseInterval);
                            Swal.fire({
